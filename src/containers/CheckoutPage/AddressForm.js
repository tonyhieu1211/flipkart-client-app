...
const AddressForm = (props) => {
    ...
    const onUpdateAddress = (e) => {
        ...
        console.log('payload onAddressSubmit ',payload);
        dispatch(addAddress(payload));
        setSubmitFlag(true);
    };

    useEffect(()=>{
        console.log('address count ',userState.address);
        console.log('submit flag', submitFlag);
        if(submitFlag){
            console.log('where are we ', userState);
            let _address = {}
            if(id){
                _address = {
                    _id:id,
                ...
                _address = userState.address.slice(userState.address.length - 1)[0];
            }
